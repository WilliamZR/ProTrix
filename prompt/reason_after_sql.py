REASON_AFTER_SQL_PROMPT = {}

REASON_AFTER_SQL_PROMPT['wikitab'] = '''This is a table question answering task. You will answer the question based on the given context.
...
REASON_AFTER_SQL_PROMPT['tabfact_small'] = '''This is a table fact verification task. You will predict whether the statement is true or false based on the given context.
You will answer the question based on the given context.You should reach a short-form answer after reasoning.
You are asked to answer the question in three steps.
1. Analyze the question and the given context. Make up a plan to answer the question.
2. Write one or more SQL to query the table for necessary information and output expected execution result.
...
REASON_AFTER_SQL_PROMPT['feverous'] = '''This is a fact verification task. You are asked to check the veracity of the claim. Both table and sentence context are provided but you are not required to use both of them. You can use either of them or both of them.
